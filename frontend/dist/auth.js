...
const savePending=d=>{localStorage.setItem('swh_pending_token',d.pendingToken);localStorage.setItem('swh_pending_user',JSON.stringify(d.user));localStorage.setItem('swh_pin_setup',d.pinSetupRequired?'1':'0');};
...
function logout(){localStorage.removeItem('swh_token');localStorage.removeItem('swh_user');localStorage.removeItem('swh_pending_token');localStorage.removeItem('swh_pending_user');localStorage.removeItem('swh_pin_setup');sessionStorage.removeItem('adminGatewayVerified');location.href='/admin/calculator/';}
async function requireUser(){try{const d=await api('/auth/me');return d.user}catch(e){logout();}}
async function requireAdmin(){try{const d=await api('/auth/me');if(d?.user?.role!=='admin'){location.href='/admin-login/';return null}return d.user;}catch(e){location.href='/admin-login/';return null;}}
