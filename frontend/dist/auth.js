const API = (window.SWH_API_BASE_URL || 'https://skincarewithhappy-backend.vercel.app/api').replace(/\/$/, '');
...
function logout(){localStorage.removeItem('swh_token');localStorage.removeItem('swh_user');localStorage.removeItem('swh_pending_token');localStorage.removeItem('swh_pending_user');location.href='/';}
async function requireUser(){try{const d=await api('/auth/me');return d.user}catch(e){logout();}}
async function requireAdmin(){const u=await requireUser();if(u?.role!=='admin'){location.href='/admin-login/';return null}return u;}
